...
function AuthScreen({ onAuthenticated, theme, onToggleTheme }: { onAuthenticated: (user: User) => void; theme: Theme; onToggleTheme: () => void }) {
  ...
  return <main className="auth"><ThemeButton theme={theme} onToggle={onToggleTheme} /><section className="auth-card"><div className="brand-mark">S</div><h1>SharedNotes</h1><p>Your writing, available wherever you are.</p><form onSubmit={submit}><label>Email<input required type="email" autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} /></label><label>Password<input required minLength={8} type="password" autoComplete={mode === 'login' ? 'current-password' : 'new-password'} value={password} onChange={(e) => setPassword(e.target.value)} /></label>{error && <p className="error">{error}</p>}<button className="primary" disabled={working}>{working ? 'Please wait…' : mode === 'login' ? 'Sign in' : 'Create account'}</button></form><button className="link-button" onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setError(''); }}>{mode === 'login' ? 'New here? Create an account' : 'Already have an account? Sign in'}</button></section></main>;
}
